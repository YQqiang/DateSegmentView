...
extension DateSegment {
    fileprivate func setupUI() {
        backgroundColor = UIColor(colorHex: 0x14487D)
        if type == .month {
            items.removeFirst()
        } else if type == .year {
            items.removeFirst()
        ...
        toolBar.backgroundColor = UIColor.bgColor()
        ...
        cancelBtn.setTitleColor(UIColor.barPriorityAColor(), for: .normal)
        ...
        confirmBtn.setTitleColor(UIColor.barPriorityAColor(), for: .normal)
